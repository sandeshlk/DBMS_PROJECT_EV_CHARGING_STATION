...
if (isset($_POST['search_button'])) {
    ...
    $sql = "SELECT S.S_NAME, C.C_NAME, L.ADDRESS, L.CITY, L.LINK, FE.NO_OF_SLOTS, FE.AMENITIES, FE.CHARGER_TYPE, FE.COST_PER_UNIT, S.TIMING_OPEN, S.TIMING_CLOSE FROM  station S, location L, features FE, company C WHERE S.S_ID = L.S_ID AND L.S_ID = FE.S_ID AND S.C_ID = C.C_ID AND (S.S_NAME LIKE '%$search%' OR L.ADDRESS LIKE '%$search%' OR L.CITY LIKE '%$search%') AND L.CITY = 'AHMEDABAD'";
} else {
    // Query to fetch all stations
    $sql = "SELECT S.S_NAME, C.C_NAME, L.ADDRESS, L.CITY, L.LINK, FE.NO_OF_SLOTS, FE.AMENITIES, FE.CHARGER_TYPE, FE.COST_PER_UNIT, S.TIMING_OPEN, S.TIMING_CLOSE FROM  station S, location L, features FE, company C WHERE S.S_ID = L.S_ID AND L.S_ID = FE.S_ID AND S.C_ID = C.C_ID AND L.CITY = 'AHMEDABAD'";
}
